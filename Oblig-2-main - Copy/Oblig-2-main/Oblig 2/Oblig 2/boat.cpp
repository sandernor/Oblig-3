...
void BOAT::init(GLint matrixUniform)
{
...
void BOAT::move(float x, float y, float z)
{
...
void BOAT::follow(player *player1)
{
    if (player1->boat == true)
    {
        if(player1->m_xyz[1] < -0.95)
            move(-m_xyz[0]+player1->m_xyz[0], -m_xyz[1]+player1->m_xyz[1], -m_xyz[2]+player1->m_xyz[2]);
        else
            move(-m_xyz[0], -m_xyz[1]-1, -m_xyz[2]);
    }
}
